...
export class CashOutService {
  ...
  async create(gameId: number, createCashOutDto: CreateCashOutDto) {
    ...
    const playersBuyIns = game.buyIns.filter((buyIn) => buyIn.player.id === createCashOutDto.playerId);
    if (!playersBuyIns.length) {
      throw new Error('O jogador não está no jogo');
    }

    const buyInChips = game.buyIns.reduce(
      (acc, cur) => acc + cur.chips,
      0
    );
    ...
    const cashOutChips = game.cashOuts.reduce(
      (acc, cur) => acc + cur.chips,
      0
    );
    ...
    const cashOut = new CashOut({ player, game, chips: createCashOutDto.chips });
